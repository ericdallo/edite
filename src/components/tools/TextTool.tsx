...
import { FontField, FontList } from './FontPicker';
...
export function TextTool() {
  ...
  const addText = (style?: { fontFamily: string }) => {
    const s = useEditorStore.getState();
    if (!s.projectId) s.newProject({ name: 'Untitled project' });
    useEditorStore.getState().addTextClip(style ? { style } : undefined);
  };
    ...
    return (
      <div className="space-y-3">
        <p className="text-sm leading-relaxed text-ink-faint">
          Pick a font to drop a text overlay on the canvas. You can restyle, reposition and time it
          afterwards.
        </p>
        <FontList onPick={(fontFamily) => addText({ fontFamily })} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <Field label="Font">
        <FontField value={text.fontFamily} onPick={(f) => updateText(clip.id, { fontFamily: f })} />
      </Field>
      ...
      <button
        onClick={() => updateText(clip.id, { shadow: !text.shadow })}
        aria-pressed={text.shadow}
        className={cn(
      ...
      >
        <Sparkles size={15} />
        <span>Shadow</span>
        <span className="ml-auto text-xs text-ink-faint">{text.shadow ? 'On' : 'Off'}</span>
      </button>
