 ...
 */
export function pwLinearExpr(points: { t: number; v: number }[]): string {
  ...
  for (let i = points.length - 1; i >= 1; i--) {
    ...
    const b = points[i];
    const span = b.t - a.t;
    const seg = span <= 1e-9 ? f(b.v) : `(${f(a.v)}+(${f(b.v - a.v)})*(t-${f(a.t)})/${f(span)})`;
    expr = `if(lt(t,${f(b.t)}),${seg},${expr})`;
  }
  return `if(lt(t,${f(points[0].t)}),${f(points[0].v)},${expr})`;
}
